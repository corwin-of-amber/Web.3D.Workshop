...
var TrackballControls = function ( object, domElement ) {
	...
	this.rotate = function(axis) {
		_lastAxis.copy(axis);
		_lastAngle = Math.max(this.minSpeed, _lastAngle);
		this.currSpeed = _lastAngle;
	}
	...
	this.rotateCamera = ( function () {
		...
		return function rotateCamera(delta) {
			...
			if ( angle ) {
				...
				quaternion.setFromAxisAngle( axis, angle * delta );
                ...
                if (_lastAngle > this.minSpeed) {
					_lastAngle *= Math.sqrt( 1.0 - _this.dynamicDampingFactor );
					_lastAngle = Math.max(this.minSpeed, _lastAngle);
				}
				_eye.copy( _this.object.position ).sub( _this.target );
				quaternion.setFromAxisAngle( _lastAxis, _lastAngle * delta );
				_eye.applyQuaternion( quaternion );
				_this.object.up.applyQuaternion( quaternion );
			...
			this.currSpeed = Math.max(_lastAngle, Math.abs(_zoomEnd.y - _zoomStart.y));
		};
	...
	function mousewheel( event ) {
		...
		if (event.deltaX) {
			_this.rotate(new Vector3(0, event.deltaX > 0 ? 1 : -1, 0));
			_lastAngle += Math.abs(event.deltaX) * (1 - _lastAngle) / 3000;
		}
